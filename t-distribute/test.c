...
int main(void) {
  ...
  for (int t = 0 ; t < TRIALS ; t++) {
    #pragma omp target
    #pragma omp teams num_teams(512)
    #pragma omp distribute dist_schedule(static,512)
    for (int i = 0 ; i < 512 ; i++)
    {
      A[i] += C[i]; // += 1 per position
    }
  ...
  for (int t = 0 ; t < TRIALS ; t++) {
    #pragma omp target
    #pragma omp teams num_teams(512)
    #pragma omp distribute dist_schedule(static,chunkSize)
    for (int i = 0 ; i < 512 ; i++)
    {
      A[i] += C[i]; // += 1 per position
    }
  ...
  for (int t = 0 ; t < TRIALS ; t++) {
    #pragma omp target
    #pragma omp teams num_teams(256)
    #pragma omp distribute dist_schedule(static,500)
    for (int i = 0 ; i < 500 ; i++)
    {
      A[i] += C[i]; // += 1 per position
    }
...
#pragma omp distribute dist_schedule(static,1)
  ...
  for (int t = 0 ; t < TRIALS ; t++) {
    #pragma omp target
    #pragma omp teams num_teams(256)
#pragma omp distribute dist_schedule(static,123)
    for (int i = 0 ; i < 123 ; i++)
    {
      A[i] += C[i]; // += 1 per position
    }
  ...
  for (int t = 0 ; t < TRIALS ; t++) {
    ...
    #pragma omp teams num_teams(256)
    {
      #pragma omp distribute private(p,q)
      for(int i = 0 ; i < N ; i++) {
	p = 2;
	q = 3;
	A[i] += p;
...
#pragma omp teams num_teams(64)
    {
      #pragma omp distribute firstprivate(p,q)
...
#pragma omp distribute lastprivate(lastpriv)
